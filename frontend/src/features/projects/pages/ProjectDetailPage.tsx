    ...
    <div className="px-4 pt-5 max-w-[1600px] mx-auto">
      ...
      <div className="mb-6">
        <button
          ...
          className="text-sm text-muted hover:text-fg transition-colors mb-3 flex items-center gap-1"
        >
          ...
          <div className="flex items-center gap-2 flex-wrap justify-end">
            {project.githubUrl && (
              <a
                ...
                className="px-3 py-2 rounded-xl bg-surface hover:bg-elevated border border-border text-sm transition-all duration-200 flex items-center gap-2"
              >
            ...
            <button
              onClick={openEdit}
              className="px-3 py-2 rounded-xl bg-surface hover:bg-elevated border border-border text-sm transition-all duration-200"
            >
              {t('projects.edit')}
            </button>
          </div>
      ...
      {/* Tabs: Board (default) | Sprints | Performance | Activity */}
        ...
        {TABS.map((tb) => (
          <button
            ...
            className={`px-4 py-2 text-sm font-semibold border-b-2 -mb-px transition-colors whitespace-nowrap ${
              tab === tb.key ? 'border-blue-500 text-fg' : 'border-transparent text-muted hover:text-fg'
            }`}
          >
            {tb.label}
          </button>
      ...
      {showEdit && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setShowEdit(false)}>
          <div className="bg-surface rounded-2xl p-6 w-full max-w-md border border-border" onClick={(e: React.MouseEvent) => e.stopPropagation()}>
            <h3 className="text-lg font-bold mb-4">{t('projects.editProject')}</h3>
            <div className="flex flex-col gap-3">
              <div>
                <label className="text-sm text-fg/80 mb-1 block">{t('projects.name')}</label>
                <input
                  type="text"
                  value={editName}
                  onChange={(e) => setEditName(e.target.value)}
                  className="w-full px-3 py-2.5 rounded-lg bg-input border border-border outline-none focus:border-blue-500 transition-colors"
                />
              </div>
              <div>
                <label className="text-sm text-fg/80 mb-1 block">{t('board.projectKey')}</label>
                <input
                  type="text"
                  value={editKey}
                  ...
                  maxLength={6}
                  className="w-full px-3 py-2.5 rounded-lg bg-input border border-border outline-none focus:border-blue-500 transition-colors font-mono uppercase tracking-wide"
                />
              </div>
              <div>
                <label className="text-sm text-fg/80 mb-1 block">{t('projects.description')}</label>
                <textarea
                  value={editDesc}
                  onChange={(e) => setEditDesc(e.target.value)}
                  rows={2}
                  className="w-full px-3 py-2.5 rounded-lg bg-input border border-border outline-none focus:border-blue-500 transition-colors resize-none"
                />
              </div>
              <div>
                <label className="text-sm text-fg/80 mb-1 block">{t('projects.githubUrl')}</label>
                <input
                  type="url"
                  value={editGithub}
              ...
              <div>
                <label className="text-sm text-fg/80 mb-1 block">{t('projects.color')}</label>
                <div className="flex gap-2">
                  {['#3b82f6', '#8b5cf6', '#ec4899', '#ef4444', '#f97316', '#eab308', '#22c55e', '#06b6d4'].map((c) => (
                    <button
                      key={c}
                      onClick={() => setEditColor(c)}
                      className={`w-8 h-8 rounded-full transition-all ${editColor === c ? 'ring-2 ring-fg ring-offset-2 ring-offset-surface scale-110' : 'hover:scale-110'}`}
              ...
              {projectId && (
                <div className="border-t border-border pt-3">
                  <MembersBar projectId={projectId} myRole={project.role} />
                </div>
              )}

              <div className="flex gap-3 mt-1">
                <button onClick={() => setShowEdit(false)} className="flex-1 py-2.5 rounded-xl bg-elevated hover:bg-border font-semibold transition-colors">
                  {t('projects.cancel')}
                </button>
                <button onClick={handleSaveEdit} className="flex-1 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-semibold transition-all duration-200 shadow-lg shadow-blue-600/20">
                  {t('projects.save')}
                </button>
              </div>
              ...
              <div className="border-t border-border mt-2 pt-3">
                {!confirmDelete ? (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => setConfirmDelete(false)} className="text-sm text-muted hover:text-fg">{t('projects.deleteNo')}</button>
                  </div>
                )}
              </div>
            </div>
